...
private:
  ...
  // Custom search
  template <typename Key> T * searchBestBin(T * root, const Key & key)
  {
    while (root) {
      if (key < *root)
      {
        // try to go Left
        // if mLeft leads to nullptr or left is smaller than key
        T * leftRoot = root->mLeft;
        if (root->mLeft == nullptr || key > *leftRoot)
        {
          // place here
          return root;
        }
        else // can still keep trying
        {
          root = root->mLeft;
        }
      }
    ...
    // exits loop if root == nullptr
    // root == nullptr means no valid bins available
    // if root == nullptr, outside program will make a new bin
    return root;
  }
/*
  template <typename Key> T * firstBinInsert( T * root, const Key & key)

  template <typename Key> T * searchFirstBin( T * root, const Key & key)
  {
    while (root)
    {
      if (key < *root)
...
public:
  ...
  //Custom Search
  template <typename Key> T * searchBestBin(const Key & key)
  {
    return searchBestBin(mRoot, key);
  }
